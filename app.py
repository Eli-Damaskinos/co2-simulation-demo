...
st.set_page_config(
    page_title="CO₂ Storage Simulator",
    page_icon="◫",
    layout="wide",
)
...
st.title("CO₂ Storage Simulator")
st.caption(
    "Explore how one storage configuration responds to CO₂ production, "
    "trailer collections and operational interruptions."
)
...
st.caption("Set the normal production rate in the sidebar. Add interruptions only if needed.")
with st.expander("Production interruptions (optional)"):
    st.write(
...
with st.expander("Edit individual collection opportunities"):
...
with st.expander("Collection disruptions (optional)"):
...
if "single_results" in st.session_state:
    ...
    st.caption("Results describe the last completed run. Change the inputs and run again to update them.")
    row1 = st.columns(4)
    ...
    usable_capacity = sum(tank.capacity - tank.minimum for tank in result_tanks)
    st.info(
        f"{len(result_tanks)} tanks | "
        f"{nominal_capacity:.1f} t nominal capacity | "
        f"{usable_capacity:.1f} t usable capacity | "
        f"{metrics['final_storage']:.1f} t remaining at the end"
    )
    ...
    with st.expander("Operational details and diagnostics"):
        row2 = st.columns(3)
        row2[0].metric("Final storage", f"{metrics['final_storage']:.1f} t")
        row2[1].metric("Production shortfall", f"{metrics['production_shortfall']:.1f} t")
        row2[2].metric("Storage-constrained time", f"{metrics['storage_constrained_hours']:.1f} h")

        row3 = st.columns(3)
        row3[0].metric("Call opportunities", metrics["candidate_collection_slots"])
        row3[1].metric("Called", metrics["called_collections"])
        row3[2].metric("Completed", metrics["completed_collections"])

        row4 = st.columns(3)
        row4[0].metric(
            "Not called", metrics["not_called_insufficient_inventory"],
            help="No single certified tank held the full trailer load.",
        )
        row4[1].metric("Missed after call", metrics["missed_collections"])
        row4[2].metric("Pending", metrics["pending_collections_at_end"])
        ...
        row5[1].metric("Maximum outstanding", metrics["maximum_outstanding_collections"])
        st.caption(f"Mass-balance error: {metrics['mass_balance_error']:.6f} t")
